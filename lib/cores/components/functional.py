"""
A collection of functional methods for defining instruction and
...
def conditional_jump(
    ...
    assert not isinstance(ins.get_state_by_operand_id(0), Box)
...
def map_vec(core: BaseCore, f, ins: Instruction, dtype: npt.DTypeLike = np.int32):
    ...
    if len(reg0) == 0 and len(reg1) == 0:
        core.set_reg(dst, Box(np.array([])))
        return
    # pass through if either length is zero
    elif len(reg0) == 0 or len(reg1) == 0:
        core.set_reg(dst, Box(reg0 if len(reg0) > 0 else reg1))
        return
