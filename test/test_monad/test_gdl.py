...
def setup(size: int = 32):
    ...
    test_list = np.arange(size, dtype=np.int32)
...
def test_gdl_fetch_edge_cases():
    ...
    def aux(mem: MemSystem, size: int):
        ...
        assert np.all(mem.fetch_gdl_at(0, 0, 0, 1, 0) == t[0:size])
        assert np.all(mem.fetch_gdl_at(0, 0, 0, 1, 1) == t[size:2*size])
        assert np.all(mem.fetch_gdl_at(0, 0, 0, 1, 2) == t[2*size:3*size])
        assert np.all(mem.fetch_gdl_at(0, 0, 0, 1, 3) == t[3*size:4*size])
        assert np.all(mem.fetch_gdl_at(0, 0, 0, 1, 4) == t[4*size:5*size])
