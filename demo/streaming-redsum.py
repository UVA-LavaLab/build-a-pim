...
if __name__ == "__main__":
    ...
    test_list: npt.NDArray[np.int32] = np.array(
        [random.randint(0, 5) for _ in range(vec_len)], dtype=np.int32
    )
    slice_len = int(len(test_list) / mem.num_banks)
    ...
    tl1id, r_op1 = pim_device_place_data(mem, cores, test_list, 0)
    ...
    all_match = sum([core.get_reg(core.registers[0]) for core in cores]) == sum(test_list)
    print("All matching?", all_match)
    if not all_match:
        print("output", sum([core.get_reg(core.registers[0]) for core in cores]))
        print("expected", sum(test_list))
